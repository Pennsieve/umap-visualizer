...
<template>
  <div class="control-panel">
    ...
    <div class="color-option">
      <div class="legend">
        <h4>{{ localColorMode === 'species' ? 'Species' : 'Sex' }} Legend</h4>
        <div v-if="localColorMode === 'species'">
          <div class="legend-item" v-for="(color, species) in speciesColors" :key="species">
            <div class="legend-color" :style="{ backgroundColor: rgbToHex(color) }"></div>
...
function regenerate() {
  emit('update:pointCount', localPointCount.value);
  emit('regenerate');
}
